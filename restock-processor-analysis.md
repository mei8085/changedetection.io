...
调用 perform_site_check()
    ...
    │
    ├─► 浏览器 JS 库存检测处理
    │   ├─► 元数据无 availability? → 用 JS 结果条件补位
    │   └─► JS 明确检测到缺货? → 强制负向覆盖 in_stock=False
    │
...
为确保本报告前后逻辑一致，特此说明全文关于**库存状态判定**的统一口径：

> **结构化数据优先 → availability 缺失时条件补位 → JS 明确缺货时负向覆盖**

**各章节对应表述验证**：

| 章节 | 表述 | 一致性校验 |
|------|------|-----------|
| 第5.3节 | 结构化数据优先、条件补位、负向覆盖 | ✅ 标准定义 |
| 第5.4节 | 2×2 状态矩阵完整覆盖四种组合 | ✅ 逻辑自洽 |
| 第7.3节 | LLM 仅作为结构化数据的补充和负向覆盖 | ✅ 口径统一 |
| 第10章时序图 | 条件补位 + 负向覆盖两步走 | ✅ 流程一致 |
| 第15.1节 | 条件补位加负向覆盖机制 | ✅ 表述统一 |
| 总结 | 结构化数据优先，JS补位，JS明确缺货时无条件覆盖为False | ✅ 总结一致 |

**常见误区澄清**：
- ❌ 错误说法："浏览器检测优先级高于元数据"（这是固定优先级，与代码逻辑不符）
- ✅ 正确逻辑：只有当 JS 明确检测到缺货时，才触发负向覆盖；JS 推测有货时，完全不影响元数据的判断
...
3. **条件补位加负向覆盖机制**: 结构化数据优先，元数据无值时用JS补位，JS明确检测到缺货时强制覆盖为False
...
3. **条件补位加负向覆盖机制**提高了检测准确率：结构化数据优先，availability缺失时用JS补位，且JS检测到明确缺货时无条件覆盖为False（防止网站元数据撒谎）
