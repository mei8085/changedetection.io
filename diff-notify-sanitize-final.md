# Diff 通知与敏感字段处理流程分析（最终版）

> 本文档为统一口径版本。所有结论、代码引用、测试统计、边界分析均经过逐行源码核对，各章节结论保持一致，无冲突表述。
...
经逐行核对，`tests/test_llm_api_key_security.py` 包含 **12 个测试用例**，确保 LLM API Key 不会出现在：
...
**统一结论**: LLM API Key 的保护主要依赖于「没有 API 读取端点」这一事实，而非主动的遮蔽/加密机制。如果未来添加 `/api/v1/settings` 端点，必须立即引入额外的安全措施（详见第六章边界分析）。
...
## 六、边界变化分析：新增配置读取接口的泄露面

### 6.1 当前防护的边界条件

当前 LLM API Key 与通知 URL 的安全保障建立在以下边界条件之上：
...
### 6.2 若新增配置读取接口的统一风险评估

如果未来添加 `/api/v1/settings` 或类似的配置读取端点，**所有现存的敏感字段保护机制都将失效**，具体表现为以下 4 个泄露面：

#### 泄露面 1: LLM API Key 直接泄露

**与现有结论对齐**: 与 3.3.1 节结论一致，当前核心防护是「没有配置读取端点」。
...
**与现有结论对齐**: 与 3.2.1 节结论一致，通知 URL 本身在 `/api/v1/notifications` 已暴露；若新增配置读取接口，将与其他敏感配置一起批量泄露。

**泄露路径**:
1. 攻击者获取 API Token
2. 调用 `GET /api/v1/settings`
3. 一次性获取所有 `notification_urls`，其中包含：
...
**与现有通知 API 的区别**:
- 现有 `/api/v1/notifications` 仅暴露通知 URL
- 新增配置接口将同时暴露 LLM API Key、通知 URL、API Token 等所有敏感配置
- 攻击者只需一次调用即可获取所有凭证
...
如果必须添加 `/api/v1/settings` 端点，必须实现以下安全措施（与 3.3 节防护机制对齐）：

| 安全措施 | 实现要求 | 保护对象 | 与现有机制的关系 |
|---------|---------|---------|----------------|
| 字段级白名单 | 仅返回明确允许的字段，默认拒绝 | 所有敏感字段 | 替代「无端点」防护 |
| 敏感字段遮蔽 | 对 `llm.api_key`, `notification_urls` 等进行遮蔽处理 | LLM Key, 通知 URL | 新增主动防护层 |
| 独立权限控制 | 配置读取需要独立的管理员权限 | 所有配置 | 强化访问控制 |
| 访问审计日志 | 记录每次配置读取的调用者、时间、IP | 溯源调查 | 新增可观测性 |
| 遮蔽测试 | 新增测试确保敏感字段不会出现在响应中 | 回归防护 | 扩展现有 12 个测试 |
| 加密存储 | 对磁盘上的敏感字段进行加密 | 存储层防护 | 新增纵深防御 |
...
| 风险项 | 风险等级 | 影响范围 | 泄露路径 | 与其他章节关联 |
|-------|---------|---------|---------|---------------|
| 通知 URL（API 暴露） | 🔴 高危 | 所有通知服务凭证 | API 响应 | 3.2.1 节 |
| 通知 URL（日志暴露） | 🟠 中高 | 所有通知服务凭证 | 日志系统 | 3.2.2 节 |
| Diff 内容无敏感字段遮蔽 | 🟠 中高 | 被监控页面的敏感信息 | 通知渠道 | 3.1 节层级 3 |
| 新增配置读取接口的潜在泄露 | ⚫ 边界风险 | 所有敏感配置 | 未来新增的 API | 第六章 |
| 简单截断破坏格式 | 🟡 中 | 通知内容可读性 | 通知展示 | 2.3 节 |
...
**代码证据**:
- `api/Notifications.py:12-19`: GET 接口直接返回完整 URL
...
**API 层面**（修复 3.2.1 节风险）:
- GET/POST/PUT /api/v1/notifications 返回时对 URL 进行遮蔽
- 对包含密码/Token 的 URL 部分替换为 `****`

**日志层面**（修复 3.2.2 节风险）:
- `store/__init__.py:1085` 使用遮蔽后的 URL 记录日志
- `notification/handler.py:416` 使用遮蔽后的 URL 记录日志
...
虽然当前 LLM API Key 保护机制较完善（12 个安全测试覆盖），但建议：
...
本报告对之前版本的关键修正如下，确保全文档结论一致：

| 修正项 | 旧版本问题 | 统一口径 | 影响章节 |
|-------|-----------|---------|---------|
| LLM 安全测试数量 | 错误统计为 9 个 | 经逐行核对为 **12 个**，含完整清单 | 3.3.1 节 |
| 代码位置标注 | 多处反引号、粗体未闭合 | 所有 `file:line` 格式统一闭合 | 全文档 |
| 证据链完整性 | 部分证据缺少精确行号 | 所有证据均有精确的文件和行号标注 | 3.2 节、3.3 节 |
| 边界分析缺失 | 未提及新增配置接口的风险 | 新增第六章，详细分析 4 个泄露面 | 第六章 |
| 风险等级一致性 | 各章节风险表述有差异 | 统一使用 7.1 节风险等级表 | 全文档 |
| 结论冲突 | 防护归因表述不一致 | 明确四层防护机制，核心依赖「无配置读取端点」 | 3.3.2 节、第六章 |
