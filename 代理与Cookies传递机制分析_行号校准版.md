# 代理与 Cookies 传递机制分析（行号校准版）

## 一、概述

本报告针对 `requests`、`playwright`、`puppeteer` 三条抓取通道，梳理代理与 cookies 的传递机制。所有文件路径统一为从仓库根目录开始的相对路径，每条结论均附可直接复核的代码片段。
...
**代码位置**：`changedetectionio/content_fetchers/playwright.py:183-215, 285-293`
...
**适配特点**：支持 HTTP/HTTPS 代理认证，SOCKS5 认证不支持（官方限制，见第280行注释）

#### Puppeteer 抓取器

**代理注入方式**：URL 参数追加 `--proxy-server` + `page.authenticate()`

**代码位置**：`changedetectionio/content_fetchers/puppeteer.py:197-225, 352-363`
...
# run 方法中使用
await self.page.setExtraHTTPHeaders(request_headers)  # 第352行
...
> **关键结论**：标签级文件配置的优先级最高（6），其次是监控级文件（5）、全局文件（4）、全局基础配置（3）、监控级配置（2），最后是系统默认 User-Agent（1）。
...
**代码位置**：`changedetectionio/content_fetchers/requests.py:59, 96`
...
context = await browser.new_context(
    accept_downloads=False,
    bypass_csp=True,
    extra_http_headers=request_headers,  # 第288行
    ignore_https_errors=True,
    proxy=self.proxy,
    service_workers=os.getenv('PLAYWRIGHT_SERVICE_WORKERS', 'allow'),
    user_agent=manage_user_agent(headers=request_headers),
...
```python
    ...
    try:
        if update_handler and hasattr(update_handler, 'fetcher') and update_handler.fetcher:
            await update_handler.fetcher.quit(watch=watch)  # 第616行
    ...
    if update_handler:
        if hasattr(update_handler, 'fetcher') and update_handler.fetcher:
            update_handler.fetcher.clear_content()  # 第626行
...
```python
    ...
    try:
        if hasattr(self, 'page') and self.page:
            ...
            await asyncio.wait_for(self.page.close(), timeout=5.0)  # 第425行
    except asyncio.TimeoutError:
        logger.warning(f"Timed out closing page for {url} (5s)")

    try:
        if context:
            await asyncio.wait_for(context.close(), timeout=5.0)  # 第436行
    except asyncio.TimeoutError:
        logger.warning(f"Timed out closing context for {url} (5s)")

    try:
        if browser:
            await asyncio.wait_for(browser.close(), timeout=5.0)  # 第447行
    except asyncio.TimeoutError:
...
```python
async def quit(self, watch=None):  # 第227行
...
**代码位置**：`changedetectionio/content_fetchers/requests.py`（继承自 base.py）

```python
# Requests 继承自 base.py 的 quit 抽象方法
# base.py:139-140
@abstractmethod
...
| quit 抽象方法 | base.py 定义 | `changedetectionio/content_fetchers/base.py:139` |
...
| 结论描述 | 旧行号 | 校准后行号 | 文件路径 |
|----------|--------|-----------|----------|
| 全局代理配置 | 32 | 31 | `changedetectionio/model/App.py` |
| get_preferred_proxy_for_watch | 855-886 | 855-886 | `changedetectionio/store/__init__.py` |
| system_http_proxy | 61-62 | 61-62 | `changedetectionio/content_fetchers/base.py` |
| call_browser | 117-260 | 117 | `changedetectionio/processors/base.py` |
| 跳过代理代码 | 179-183 | 179-183 | `changedetectionio/processors/base.py` |
| Headers 合并 | 200-208 | 198-208 | `changedetectionio/processors/base.py` |
| get_all_headers_in_textfile | 900-934 | 900-934 | `changedetectionio/store/__init__.py` |
| Requests 代理代码 | 51-58 | 45-57 | `changedetectionio/content_fetchers/requests.py` |
| Playwright __init__ | 183 | 183 | `changedetectionio/content_fetchers/playwright.py` |
| Playwright 代理解析 | 183-215 | 196-215 | `changedetectionio/content_fetchers/playwright.py` |
| Playwright new_context | 285-293 | 285-293 | `changedetectionio/content_fetchers/playwright.py` |
| Playwright 关闭 | 420-458 | 420-454 | `changedetectionio/content_fetchers/playwright.py` |
| Puppeteer __init__ | 197 | 197 | `changedetectionio/content_fetchers/puppeteer.py` |
| Puppeteer 代理代码 | 209-225 | 208-225 | `changedetectionio/content_fetchers/puppeteer.py` |
| Puppeteer setExtraHTTPHeaders | 351-352 | 352 | `changedetectionio/content_fetchers/puppeteer.py` |
| Puppeteer authenticate | 358-363 | 363 | `changedetectionio/content_fetchers/puppeteer.py` |
| Puppeteer quit | 227-257 | 227 | `changedetectionio/content_fetchers/puppeteer.py` |
| clear_content | 105-116 | 105-116 | `changedetectionio/content_fetchers/base.py` |
| quit 抽象方法 | 139 | 139 | `changedetectionio/content_fetchers/base.py` |
| Worker 清理 | 605-680 | 605-640 | `changedetectionio/worker.py` |