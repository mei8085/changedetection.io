...
Changedetection.io 的文件型 DataStore 采用原子写入、分层存储和即时提交机制，**在工程层面降低** Watch 配置数据与历史快照数据不一致的风险。

**三大核心边界前提**：
1. **单文件系统调用语义**：仅 `os.replace()` 单个系统调用具备原子性，多步操作（如 write+flush+fsync）不保证原子性
2. **跨文件一致性不保证**：watch.json、history.txt、快照文件为独立写入，不保证跨文件的事务一致性
3. **异常场景仅降风险**：原子写入等机制降低崩溃损坏概率，但不能 100% 规避所有硬件/OS 级异常场景

**额外限制**：本机制仅适配单进程场景，不支持多进程并发写入。多进程场景下需使用 Redis/SQL 后端。
...
4. 原子替换 (os.replace)
   ...
   - POSIX 规范定义为原子系统调用：内核层面不可被信号中断
   - 主流本地文件系统（ext4/xfs/NTFS）下，原文件要么完整保留，要么被新文件完全替换
   - 仅在单系统调用层面具备原子语义，不扩展到多步操作链

5. 新文件目录 fsync（尝试确保文件名元数据持久化，依赖具体文件系统实现）
   - 仅针对新创建的文件
...
**工程保证（带前提边界）**：
- 单进程 + 主流本地文件系统 + 无硬件故障场景下，`os.replace()` 临时文件替换模式**显著降低** watch.json 出现半写损坏的概率
- 前提 1：文件系统正确实现 POSIX rename 原子语义
- 前提 2：无磁盘坏道、电源异常等硬件级故障
- 前提 3：无虚拟化层快照/迁移等存储栈异常

**重要限制**：`os.replace()` 的原子性仅局限于单个系统调用本身，不能解决多进程场景下的竞态条件（如进程 A 读取后进程 B 并发写入导致的丢失更新问题）。
...
1. ✅ **单进程下 watch.json 损坏概率极低**：原子写入 + 临时文件替换模式，主流本地文件系统下极大降低半写风险
...
**报告生成时间**：2026-05-17
**修订版本**：v3 - 修正 history.txt 写入语义表述，严格对应源码行为
